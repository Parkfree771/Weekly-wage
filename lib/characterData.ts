// 캐릭터 조회 데이터 파서
// LostArk Armory API 응답을 캐릭터 정보 형태로 가공

import { BRACELET_ALL_OPTIONS } from './characterTables';
...
export function parseGems(gemsData: any): GemInfo[] {
  ...
  // 실제 API 구조: gemsData.Effects = { Description, Skills: [...] }
  // Skills[] 가 GemSlot 으로 매칭되는 스킬 정보 (이름·아이콘) 배열
  const skillEffects: any[] = Array.isArray(gemsData?.Effects?.Skills)
    ? gemsData.Effects.Skills
    : Array.isArray(gemsData?.Effects)
      ? gemsData.Effects
      : [];

  // HTML 태그 제거 + 앞부분 "[직업명]" 브래킷 제거 (예: "[슬레이어] 볼케이노 이럽션" → "볼케이노 이럽션")
  const cleanSkillName = (raw: string): string => {
    let s = String(raw || '').replace(/<[^>]*>/g, '').trim();
    // 처음 나오는 "[...]" 토큰들을 모두 제거 (보통 직업명·태그)
    while (/^\s*\[[^\]]*\]\s*/.test(s)) {
      s = s.replace(/^\s*\[[^\]]*\]\s*/, '');
    }
    return s.trim();
  ...
  // 한 보석에 연결된 스킬 정보(이름·아이콘)를 추출
  // LostArk API: ArmoryGem.Effects[].Name = 스킬명, Effects[].Icon = 스킬 아이콘
  const resolveSkill = (gem: any): { name: string; icon?: string } => {
    // 1) Skills 배열에서 GemSlot으로 매칭 (느슨 비교)
    // eslint-disable-next-line eqeqeq
    const eff = skillEffects.find(e => e && e.GemSlot == gem.Slot);
    if (eff) {
      const name = cleanSkillName(eff.Name);
      if (name) return { name, icon: eff.Icon };
      // Name이 비어있어도 아이콘은 가져옴
      if (eff.Icon) return { name: '', icon: eff.Icon };
    }

    // 2) gem 자체에 SkillName 필드가 있는 경우
    const direct = gem.SkillName || gem.SkilName;
    if (direct) return { name: cleanSkillName(direct) };
  ...
  for (const gem of gemsData.Gems) {
    ...
    if (name.includes('겁화')) {
      type = '겁화';
    } else if (name.includes('작열')) {
      type = '작열';
    } else if (name.includes('광휘')) {
      // eslint-disable-next-line eqeqeq
      const eff = skillEffects.find(e => e && e.GemSlot == gem.Slot);
      const descRaw = eff
        ? (Array.isArray(eff.Description) ? eff.Description.join(' ') : String(eff.Description || ''))
        : '';
      const desc = descRaw.replace(/<[^>]*>/g, '');
      type = /재사용|쿨타임|재시전/.test(desc) ? '작열' : '겁화';
    }
