// 66112543, 66112546 아이템의 과거 14일치 데이터를 로스트아크 공식 API에서 가져와 저장하는 스크립트
...
async function fetchAndSaveHistoricalData(itemId, itemName) {
  try {
    ...
    console.log(`  💾 Firebase에 ${historicalData.length}일치 데이터 저장 중...`);
...
async function main() {
  ...
  console.log('로스트아크 공식 API로 과거 14일치 데이터 수집 시작');
  console.log('='.repeat(60));
  ...
  console.log('작업 완료!');
  console.log('='.repeat(60));
  console.log('\n이제 /api/cron/collect-prices가 자동으로');
  console.log('이 아이템들의 가격을 수집합니다.');
  console.log('(items-to-track.ts에 이미 등록되어 있음)');
}
