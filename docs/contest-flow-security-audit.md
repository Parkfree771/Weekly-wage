...
[로그인 시] Supabase 호출 2:  fetchUserLikedIllusts(uid)
  ...
  → sessionStorage 캐시 5분
  ↓
[일러스트 카드] Firestore 호출 12: getWeaponCount(slug)
  → contestWeapons/{slug} 도큐 1번씩 read (count 필드)
  → 캐시 없음 (필요시 추후 sessionStorage 적용 가능)
```
...
Firestore: getWeapons(slug)  → contestWeapons/{slug}/items 전체 read (메타)
  ↓
Supabase 병렬 호출:
  - fetchWeaponLikeCounts(slug, weaponIds)  → 무기들 좋아요 카운트
  - fetchWeaponViewCounts(slug, weaponIds)  → 무기들 조회수
  ↓
[로그인 시] fetchUserLikedWeapons(uid, slug) → 본인이 좋아요한 무기 id
  ↓
머지 후 갤러리 표시 (카드: 이미지 + 좋아요 + 조회 + 댓글수)
```
...
Firestore:  addDoc(contestWeapons/{slug}/items)
  ...
  ↓
Firestore:  updateDoc(contestWeapons/{slug}, { count: increment(1) })
  ↓
...
사용자 무기 카드 이미지 클릭
  ↓
WeaponDetailModal 오픈 (좌 이미지 + 우 메타·좋아요·댓글)
  ↓
백그라운드:  tryIncrementWeaponView(weaponId, uid?)
  - 클라이언트 dedup: sessionStorage 'csk:viewed_weapons' 에 weaponId 있으면 skip
  - 없으면 INSERT INTO contest_weapon_views (weapon_id, viewer_key)
    - viewer_key: 로그인=`{uid}_{YYYY-MM-DD-HH}` / 비로그인=`anon_{localStorage_uuid}_{YYYY-MM-DD-HH}`
    - PK 충돌(같은 시간대 본 적 있음) → INSERT 무시 → 카운트 안 오름
    - 새 INSERT 성공 → 트리거가 view_count +1
  - sessionStorage 마킹 (실패해도)
```
...
| 비로그인 무기 조회수 | 허용 (의도) — viewer_key `anon_*` 허용 | ⚠ 의도된 동작 |
...
| Firebase 무기 도큐 likeCount 조작 | `count`/`commentCount` 필드만 increment 허용 (좋아요 필드 자체 없음) | ✅ 차단 |
...
| viewer_key 형식 | Supabase RLS (`uid_*` 또는 `anon_*` 시작) | 우회 시 어뷰징 가능 (아래 항목) |

### 2-3. 알려진 위협 / 잔여 리스크

#### A. 비로그인 조회수 어뷰징 가능
- 비로그인 사용자는 viewer_key 형식만 `anon_*` 이면 INSERT 허용
- 누가 SDK 직접 호출해서 매번 다른 uuid 로 무한 INSERT 가능
- **영향**: 무기 조회수 부풀리기 가능
- **대응**: 조회수의 정확도가 핵심 지표가 아니라 가벼운 트렌드 표시이므로 수용 (사용자가 의도적 선택). 어뷰징 시 Supabase 대시보드에서 contest_weapon_views 행 수 모니터링 권장.

#### B. 좋아요 어뷰징 (시크릿 모드 / 멀티 계정)
...
| 호출 | 횟수 |
|---|---|
| 페이지 진입 — 일러스트별 무기 갯수 12 reads × 3000 × (캐시 안 적용) | 36,000 ⚠ |
| 무기 모달 진입 — 메타 1 read × 3 모달 × 3000 | 9,000 |
| 무기 댓글 조회 — 모달 댓글 토글 시 | ~3,000 |
| **합계** | **약 48,000/일** ⚠ 한도 50K 빠듯 |

→ **개선 권장**: `getWeaponCount` 12개 호출도 sessionStorage 캐시화 또는 Supabase 로 옮기기. 이번 작업 범위 밖.
...
| 조회수 INSERT (대부분 PK 충돌로 무시) | 약 5,000 |
| **합계** | **약 35,000/일** ✅ Supabase 무제한 한도 |
...
- **현실적 위협**: 비로그인 조회수 어뷰징(낮음, 의도된 트레이드오프), Firestore 무기 카운트 read 부담(개선 여지)

### 권장 후속 작업 (이번 범위 밖)
1. **`getWeaponCount` 12개 → sessionStorage 캐시** (Firestore read 75% ↓)
2. **Firebase 콘솔 사용량 알림 활성화** (read/write/storage 80% 도달 시 메일)
3. **Supabase 사용량 알림** (egress 80% 도달 시)
4. **무기 업로드 사용자당 갯수 제한** (Firebase Functions, 어뷰저 방지)
5. **CDN 앞단 rate limit** (Cloudflare 등) — 트래픽 폭주 대비
