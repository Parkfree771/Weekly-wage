...
**2026-08-31 재대조: 최신 원본을 새로 받아 다시 돌렸고 여전히 전 항목 일치**
(지옥 11행·나락 9행·Sub 4행 모두 불일치 0). 이 시점 한섭 파일에는 아직 시즌4가 없다.
...
언제나 한섭 `kr_` 파일이 기준이다. 북미 수치를 표에 넣지 말 것.

## 4-4. 지역·시즌 대조 (2026-08-31)

그날 기준 각 파일의 카테고리 현황:

| 파일 | 1750 | 비고 |
|---|---|---|
| 한섭 `kr_` | 시즌3 = **1026**(지옥) / **1027**(나락) | 시즌4 아직 없음 |
| 북미 | 시즌4 = **1028**(지옥) / **1128**(나락) | 북미 **시즌3에는 1750 자체가 없다**(최고 1730 = 1025/1125) |

### 한섭 시즌3 1750 vs 북미 시즌4 1750

- **지옥**: 팔찌 한 행 빼고 **전 행·전 단계 수량이 완전히 같다.** 층 기본 보상(Sub) 44항목도 전부 일치.
  - 팔찌만 상자 아이템 자체가 다르다: 한섭 `고대 팔찌 상자`(65811040) vs 북미 `Ancient Bracelet Chest`(65811920).
    안에 든 팔찌는 같은 아이템(찬란한 구원자의 팔찌 213400033 ×1)이고 개수만 북미가 많다
    (Base 4→9 … Max 150→220).
- **나락**: 차이가 크다.
  - 같음: 용숨·빙숨, 어빌리티스톤, 젬 선택 상자(희귀·영웅), 정련된 혼돈/운명의 돌, 귀속골드
  - 다름: 팔찌(20~750 → 45~1100, 상자 ID도 다름), 각인서 랜덤 주머니(3~100 → 5~120),
    8레벨 광휘의 보석(4/5/6 → 5/6/7)
  - **카드팩은 구조가 통째로 다르다.** 한섭은 `열망의 전설 카드 팩`(61106390) 한 행이 Level 5부터
    1/2/3/4/5/7. 북미는 세 행으로 쪼개져 있다 — `Joyful Legendary Card Pack`(61106380) 전 단계 ×1,
    `Joyful Legendary - Epic Card Pack`(61001601) Level 1부터 3~80,
    `Joyful Legendary - Rare Card Pack`(61001602) Level 1부터 5~500.
    담는 방식도 다르다: 한섭은 **31종 카드 중 확률로 1종**, 북미 61106380 은 **6종 카드팩 중 택1(선택형)**,
    나머지 둘은 확률형 묶음이다. 가치 환산 분기(확률 상자 / 선택 상자)가 달라지므로 그대로 못 옮긴다.

**결국 "목록(행 구성)" 차이는 나락 카드팩 하나뿐이다.** 지옥은 행 종류가 완전히 같고, 팔찌는 양쪽 다
있으나 상자 아이템 ID 가 서로 다르다(내용물은 같은 팔찌 ×1, 확률 1종 획득). 나머지는 전부 같은 행이다.

### 북미 시즌3 vs 한섭 시즌3 (1730 기준 — 북미 S3에 1750이 없어 최고 구간을 맞춤)

- **지옥(한섭 1015 vs 북미 1025)**: 팔찌 행 빼고 전부 일치. 팔찌는 여기서도 상자 ID가 다르고
  개수가 북미가 많다(3~120 → 8~200).
- **나락(한섭 1019 vs 북미 1125)**: 1750과 같은 양상 — 팔찌(15~600 → 40~1000),
  각인서 주머니(2~80 → 4~100), 광휘의 보석(3/4/5 → 4/5/6), 카드팩 3행 구조.

**결론: 두 지역은 지옥이 거의 같고(팔찌만 예외), 나락에서 갈린다.** 북미가 팔찌·각인서·보석·카드팩을
일관되게 더 준다. 그래도 **수치 출처는 한섭 파일** 원칙은 그대로다.

### 시즌4 미리보기로서의 값어치

한섭 시즌4가 열렸을 때 북미 시즌4 1750(1028/1128)이 **구조 예고편**은 된다. 특히 나락 카드팩이
3행 구조로 바뀌면 우리 표에 행을 추가해야 한다(`calcBoxRewardGold`·`REWARD_IMAGES`·`DISPLAY_NAMES`).
다만 개수는 지역마다 다르므로 **한섭 파일이 올라온 뒤 그 수치로 다시 뽑아야 한다.**
