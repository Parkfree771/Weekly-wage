// 새로 추가된 젬 2개의 과거 가격 데이터 수집 스크립트
...
async function collectHistoricalPrices() {
  ...
  console.log('🚀 새 젬 과거 가격 데이터 수집 시작...\n');
    ...
    try {
      ...
      for (const stat of itemData.Stats) {
        if (stat.AvgPrice > 0 && stat.Date) {
          try {
            const saveResponse = await fetch('http://localhost:3001/api/market/price-history/save', {
              method: 'POST',
              headers: {
  ...
  console.log('🎉 새 젬 과거 가격 데이터 수집 완료!');
}
