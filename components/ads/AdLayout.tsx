...
export default function AdLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      // 기존 ArkPass 사이드 레일 경로 (현재 SIDE_RAILS_DISABLED로 OFF)
      const railsAllowed = !SIDE_RAILS_DISABLED && !railsDisabled;
      setShowSideAds(railsAllowed && effective >= needed);
    };
  ...
  const railsVisible =
    ...
    (AD_PREVIEW || !!AD_SLOTS.sidebar) && !showSideAds && !railsDisabled && railsWide && !isMobile;
  ...
  const layoutStyle: React.CSSProperties =
    showSideAds || railsVisible ? { maxWidth: `${contentWidth + AD_EXTRA}px` } : {};
