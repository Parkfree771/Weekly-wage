...
export type PackageItem = {
  ...
  quantity: number;      // 수량
  /** '핫딜샵' 전용: 이 칸(상품) 하나의 가격. 단위는 글의 priceCurrency (원 또는 블크).
   *  패키지 가격(royalCrystalPrice·blueCrystalPrice)은 칸 가격의 합이다. */
  slotPrice?: number;
  icon?: string;         // 아이콘 경로
...
/** 패키지 타입 */
export type PackageType = '3+1' | '2+1' | '3+보너스' | '일반' | '가챠' | '핫딜샵';

/** 가격 통화 타입 */
export type PriceCurrency = 'cash' | 'blueCrystal';

/** 갤러리 카드 이벤트 테마. 설날·할로윈 등은 값만 늘리면 된다. */
export type PackageEventTheme = 'chuseok';
...
export type PackagePost = {
  ...
  /** 이벤트 테마 — 값이 있으면 갤러리 카드가 그 테마 옷을 입는다. 없거나 null 이면 평소 카드.
   *  기간이 지나도 글에 남는다(작년 추석 패키지와 비교할 때 오히려 구분이 된다). */
