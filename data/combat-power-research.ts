 ...
 *
 * ■ 핵심 공식:
 *   전투력(표시값) = 순수기본공격력 × 0.000288 × Π(1 + 요소%)
 *   (= 순수기본공격력 × 0.0288 × Π ÷ 100)
 *   ※ 원문의 0.0288은 내부 계산 상수, API CombatPower는 ÷100 스케일링
 *   모든 요소는 곱연산으로 처리
 *
 * ■ 기본 공격력:
 *   sqrt(힘민지 × 무기공격력 / 6)
...
// ═══════════════════════════════════════
// 1-1. T4 장비 강화 단계별 스탯 (운명의 전율 실측)
// ═══════════════════════════════════════
// ■ 핵심 발견: 모든 T4 장비(무기+방어구 5부위)의 강화 비율이 동일
//   - 1~16강: 단계당 ~2.64%~2.91% 점진적 증가
//   - 17~25강: 단계당 정확히 2.5% 증가
//   - 6부위 간 비율 차이 최대 0.004% (정수 반올림 오차)
//
// ■ 전투력 계산 방식:
//   기본 공격력 = sqrt(힘민지 × 무기공 / 6)
//   - 무기 강화: 무기공 변동 → CP × (sqrt(new무기공/old무기공) - 1)
//   - 방어구 강화: 힘민지 변동 → CP × (sqrt(1 + delta힘/total힘) - 1)
//
// ■ 실측 데이터 (운명의 전율, 1~25강):
//   투구 힘:   73903 → 139346 (1강→25강)
//   상의 힘:   59123 → 111477
//   하의 힘:   63872 → 120435
//   장갑 힘:   88684 → 167216
//   견갑 힘:   78654 → 148304
//   무기 공격력: 128059 → 241367
//
// ■ 검증 (char_a 18강, 무기 23강, CP 5067):
//   API 파싱값과 테이블 18강 값 6부위 모두 정확히 일치
//   투구 18→19: CP+13, 장갑 18→19: CP+15, 무기 23→24: CP+63
...
export const ARK_PASSIVE = {
  /**
   * 진화 비스탯 노드: 포인트당 1.0714% (실측 검증 2026-03-03)
   * - 15P 제거: CP 5067.31→4741.56 (예측 4741.47, 오차 0.002%)
   * - 10P 제거: CP 4741.56→4524.39 (예측 4524.39, 오차 0.000%)
   * - 2P 제거: CP 2895.61→2846.69 (스탯 노드, 포인트 기여 0 확인)
   *
   * ■ 진화 스탯 노드 (치명/특화/신속): 포인트 기여 0!
   *   - 1P = 50 스탯 (50 신속 등)
   *   - 스탯 교체(신속→인내, 포인트 불변) = 스탯 노드 제거와 동일한 CP 변동
   *   - 스탯 변동분은 스탯 팩터(0.03%/pt)에서 반영
   *   - 진화 140P 중 100P는 고정(비스탯), 40P는 자유 배분(스탯/비스탯 선택)
   */
  evolution: { perPoint: 1.0714, maxPoints: 140, note: '비스탯 노드만 해당. 스탯 노드 포인트 기여 0' },
  /**
   * 깨달음 포인트당 0.7% (실측 검증 2026-03-03)
   * - 100→98P: CP 5067.31→5025.58 (예측 5025.58, 오차 0.000%)
   * - 100→88P: CP→4816.93 (예측 4816.93, 오차 0.000%)
   * - 100→0P: CP→2422.08 (카르마OFF 상태 베이스)
   *
   * ■ 4단계 구조: 1~3단계(각 24P, 합계 72P) + 4단계(메인+사이드)
   *   - 각 단계는 전단계 24P 완료해야 해금
   *   - 4단계 메인 노드: 3개, 각 8P (찍으면 ×1.231 곱연산 ON/OFF)
   *   - 메인 노드 1개 이상 활성 → ×1.231 (2개/3개도 동일, 추가 증가 없음)
   *   - 메인 미활성(사이드만): 순수 0.7%/P (76P=3710.63, 78P=3744.54 모두 0.000%)
   *   - 메인 활성(80P 이상): 0.7%/P × 1.231 (88P/98P/100P 모두 0.000%)
   *   - P ≥ 80이면 4단계 메인 활성화로 간주 (시뮬레이터 기본 가정)
   */
  enlightenment: { perPoint: 0.7, maxPoints: 101, maxPercent: 70.7, mainNodeMultiplier: 1.231 },
  /**
   * 도약 포인트당 0.2% (실측 검증 2026-03-03)
   * - 70→60P: CP 5067.31→4978.41 (예측 4978.41, 오차 0.000%)
   * - 70→0P: CP→4445.01 (예측 4445.01, 오차 0.000%)
...
// 4. 카르마
// ═══════════════════════════════════════
// ■ 카르마 구조: 각 아크패시브(진화/깨달음/도약)마다 랭크(1~6)와 레벨이 있음
//   - 1랭크: 레벨 1~5, 2랭크: 레벨 6~9 ... 6랭크: 레벨 21~30
//   - 랭크가 올라야 더 높은 레벨 해금
//   - API: arkpassive.Points[].Description = "6랭크 21레벨" 형식으로 파싱 가능
//   - API의 Points[].Value (진화140, 깨달음101, 도약70)에는 카르마 랭크 보너스 포인트가 이미 포함됨
//
// ■ 전투력 반영 방식:
//   진화 카르마
//   - 1레벨당 최대 생명력 → 전투력 영향 없음
//   - 1랭크당 진화형 피해 1% 증가, 전투력 0.6% 증가. 6랭크 = 3.6%
//
//   깨달음 카르마
//   - 1레벨당 무기 공격력% 증가 → 기본 공격력 상승 (97돌/보석기본공%/팔찌 등에 의해 플레이어마다 다름)
//   - 1랭크당 깨달음 포인트 +1 → 전투력 0.7% (노드 찍을 수 있는 경우에만, 이미 API Value에 포함)
//
//   도약 카르마
//   - 1레벨당 초각성기 피해 0.5% 증가, 전투력 0.02% 증가
//     (초각성기 딜 증가 노드 미착용, 딜지분 4% 가정 시 동일한 수치)
//   - 1랭크당 도약 포인트 +2 → 전투력 0.4% (노드 찍을 수 있는 경우에만, 이미 API Value에 포함)
//
// ■ 시뮬레이터에서 별도 곱연산으로 추가해야 하는 것:
//   - 진화 카르마 랭크: rank × 0.6%
//   - 도약 카르마 레벨: level × 0.02%
//   - 깨달음 카르마 레벨: 기본 공격력에 이미 반영 → 별도 추가 불필요
//   - 깨달음/도약 랭크 포인트: API Value에 이미 포함 → 별도 추가 불필요
...
export const KARMA = {
  ...
  enlightenment: {
    /** 1레벨당 무기 공격력% → 기본 공격력 상승 (플레이어마다 다름, 별도 곱연산 불필요) */
    perLevel: 0,
    /** 1랭크당 깨달음 포인트 +1 → 0.7% (이미 API Value에 포함) */
    perRank: 0,
  },
  leap: {
    /** 1레벨당 초각성기 피해 0.5% 증가 → 전투력 0.02% 증가 */
    perLevel: 0.02,
    /** 1랭크당 도약 포인트 +2 → 0.4% (이미 API Value에 포함) */
    perRank: 0,
  },
};
...
export const ARK_GRID_EFFECT_PER_LEVEL: Record<string, { perLevel: number; affectsDpsCombatPower: boolean; poolType: 'additive' | 'standalone'; source: string }> = {
  // 실측 데이터 (구아바밤바아 기준, 2개 캐릭터 교차 검증)
  // ★ poolType: 'additive' = 다른 소스와 합산 후 하나의 곱연산 요소, 'standalone' = 독립 곱연산 요소
  '추가 피해': { perLevel: 0.08, affectsDpsCombatPower: true, poolType: 'additive', source: '합연산 풀(무기30%+악세+팔찌≈37%). 젬 제거 시: standalone -0.423% vs 실측 -0.331%, additive -0.328% → 합연산 확정' },
  '보스 피해': { perLevel: 0.083, affectsDpsCombatPower: true, poolType: 'standalone', source: '독립 곱연산. 젬 제거 시: standalone -0.412% vs 실측 -0.418% (0.006% 오차) → 독립 확정' },
  '공격력': { perLevel: 0.037, affectsDpsCombatPower: true, poolType: 'additive', source: '합연산 풀(보석공격력%+기타≈17%). 젬 제거 시: standalone -0.110% vs 실측 -0.095%, additive -0.095% → 합연산 확정' },
  '낙인력': { perLevel: 0, affectsDpsCombatPower: false, poolType: 'standalone', source: '딜러 전투력 미반영 (서포터 전용)' },
  '아군 피해 강화': { perLevel: 0.052, affectsDpsCombatPower: false, poolType: 'additive', source: '딜러 전투력 미반영 (서포터 전용). Lv.1=0.05%, Lv.3=0.15%' },
  '아군 공격 강화': { perLevel: 0.13, affectsDpsCombatPower: false, poolType: 'additive', source: '딜러 전투력 미반영 (서포터 전용). 처어단자아 Lv.8=1.04%' },
};

// ■ 그리드 젬 효과 → 전투력 반영 방식 (풀 모델, 2026-03-03 검증):
//
//   ★ 핵심: 효과별로 반영 방식이 다름!
//
//   [standalone 효과] 보스피해:
//     그리드 젬 보스피해%가 독립된 곱연산 요소로 적용
//     전투력 변화율 = (1 + new%) / (1 + old%) - 1
//     검증: 보피-5 → standalone -0.412% vs 실측 -0.418% (0.006%)
//
//   [additive 효과] 추가피해, 공격력:
//     그리드 젬 기여분이 다른 소스(무기, 악세, 팔찌 등)와 합산되어
//     하나의 곱연산 요소를 형성. 그리드 기여분 변경 시 풀 전체가 변함.
//     전투력 변화율 = (1 + (poolOther + new%) / 100) / (1 + (poolOther + old%) / 100) - 1
//     검증(추피): standalone -0.423% vs additive(37%) -0.328% vs 실측 -0.331%
//     검증(공격력): standalone -0.110% vs additive(17%) -0.095% vs 실측 -0.095%
//
//   [딜러 전투력 미반영] 낙인력, 아군피해강화, 아군공격강화:
//     서포터 전용 효과로 딜러 캐릭터의 전투력에 반영 안 됨
//
// ■ 코어 등급별 차이:
//   전설: 의지력 공급 12pt, 코어 옵션 10P/14P까지
//   유물: 의지력 공급 15pt, 코어 옵션 10P/14P/17P/18P/19P까지
//   고대: 의지력 공급 ?, 코어 옵션 10P/14P/17P/18P/19P/20P까지 (고대=유물+1% at 17P↑)
//
// ■ 젬 등급별 차이:
...
//   4. standalone: 전투력 변화율 = (1 + new%) / (1 + cur%) - 1
//      additive: 전투력 변화율 = (1 + (poolOther+new%)/100) / (1 + (poolOther+cur%)/100) - 1
//
...
export const ANALYSIS_NOTES = [
  ...
  '도약 포인트 전투력 기여 적음 (0.2%/P, 70P 풀투자 = 14% 증가)',
  '깨달음 4단계 메인 노드 ×1.231 반영 완료 (P≥80이면 활성화 가정)',
  'API에서 가져올 수 없는 데이터: 펫 방범대, 에스더 부여 상세',
  '카르마 랭크/레벨: arkpassive.Points[].Description에서 파싱 가능 (예: "6랭크 21레벨")',
];
...
export const VERIFICATION_RESULTS = {
  ...
  discovery: {
    ...
    // ── 2026-03-03 실측 검증 (진화/스탯) ──
    '진화 비스탯 계수 1.0714%/P': '15P 제거 오차 0.002%, 10P 제거 오차 0.000%. 기존 0.5%는 틀림',
    '진화 스탯 노드 포인트 기여 0': '스탯 교체(신속→인내, P 불변) = 스탯 노드 제거와 동일 CP 변동. 진화 P 자체 효과 없음',
    '진화 구조: 100 고정 + 40 자유배분': '140P 중 100P는 고정 비스탯, 40P는 스탯/비스탯 선택 가능',
    '스탯 계수 0.03% 곱연산 확정': '진화 스탯 교체 테스트(순수 스탯만 변동). CP 2895.61→2846.69, 예측 2846.72 (0.001% 오차)',
    '제압/인내/숙련 전투력 미반영': '신속→인내 교체 시 신속 감소분만 반영, 인내 증가분 0',
    // ── 2026-03-03 실측 검증 (깨달음/도약) ──
    '깨달음 0.7%/P 확정': '100→98P: 0.000%, 100→88P: 0.000%. 모든 포인트 동일 계수',
    '깨달음 4단계 메인 ×1.231': '4단계 메인 ON/OFF 스위치. 0P=2422.08, 76P=3710.63(OFF), 80P=4680.01(ON), 100P=5067.31(ON)',
    '깨달음 4단계 구조': '1~3단계 각 24P(합 72P), 4단계 메인 3개×8P. 메인 1개라도 찍으면 ×1.231 (2/3개도 동일)',
    '깨달음 메인 미활성 검증': '76P(사이드만)=3710.63: B×1.532=3710.63(0.000%). 78P=3744.54: B×1.546=3744.54(0.000%)',
    '도약 0.2%/P 확정': '70→60P=4978.41(0.000%), 70→0P=4445.01(0.000%). 카르마 보너스 없음',
    '각인 독립 곱연산': '원한 해제: 5067.31/1.18=4293.48, 실측 4294.33 (0.02% 오차)',
    '각인 돌 매트릭스 검증': '돌격대장3+질량1→돌격대장3+기습1: 예측 5055.60, 실측 5055.59 (0.0002%)',
    '카드 독립 곱연산': '세빛30각 해제: 5067.31/1.15=4406.36, 실측 4406.36 (0.00%)',
    '보석 이중 효과': '기본공격력% + 개별 곱연산. T4-8 하나 제거: 예측 4756.40, 실측 4756.36 (0.001%)',
    '보석 개별 곱연산 최종 확정': [
      '보석은 합산이 아닌 개별 곱연산: Π(1+gem_i%). 기본공격력%는 별도 곱연산 팩터.',
      '1개 제거(T4-8): 예측 4756.24, 실측 4756.36 (0.003%)',
      '2개 제거(T4-8+T4-10): 예측 4394.55, 실측 4394.53 (0.0005%)',
      '9개 제거(T4-8×9): 예측 2860.07, 실측 2860.05 (0.0007%)',
    ].join('\n'),
    '보석 기본공격력% 개별값': 'T4-6=0.45%, T4-7=0.60%, T4-8=0.80%, T4-9=1.00%, T4-10=1.20% (= 11개기준/11)',
    '추가피해 합연산': '무기+악세+그리드 출처 모두 합산 후 하나의 곱연산 팩터',
    // ── 2026-03-03 실측 검증 (질서_별 코어 옵션) ──
    '질서_별 코어옵션 전 구간 검증 (고대)': [
      '구아바밤바아 별 코어(고대 20P), 기준 CP 5067.31',
      '0P=4749.58, 10P=4803.67, 14P=4875.01, 17P=5017.70, 18P=5025.78, 19P=5035.39, 20P=5067.31',
      '10P~18P 구간 젬 불변(공29/보피9/추피37) → 순수 코어 옵션 변화만 측정',
      '테이블 예측 vs 실측: 전 구간 0.005% 이내 일치',
    ].join('\n'),
    '질서_별 고대 = 유물 + 1%': '17P 이상 구간에서만 고대 보너스 +1.00% 적용 (10P/14P는 유물과 동일)',
    // ── 2026-03-03 실측 검증 (젬 효과 풀 모델) ──
    '젬 효과 풀 모델 발견': [
      '그리드 젬 효과가 독립 곱연산이 아닌, 같은 종류의 다른 소스와 합산(additive)되는 효과가 있음',
      '★ 추가피해: 합연산 (무기30%+악세1.6%+팔찌2.5%+그리드 ≈ 37% 풀). 0.08%/Lv',
      '★ 보스피해: 독립 곱연산. 0.083%/Lv',
      '★ 공격력: 합연산 (보석공격력%+기타 ≈ 17% 풀). 0.037%/Lv',
    ].join('\n'),
    '젬 풀 모델 검증 테스트1 (추피)': '별 코어 젬 제거(20→15P), 추피-5: standalone -0.423% vs additive(37%) -0.328% vs 실측 -0.331% → 합연산',
    '젬 풀 모델 검증 테스트2 (보피)': '달 코어 젬 제거(18→13P), 보피-5: standalone -0.412% vs 실측 -0.418% (0.006%) → 독립 곱연산',
    '젬 풀 모델 검증 테스트3 (공격력)': '별 코어 젬 제거(20→15P), 공격력-3/보피-1: 공격력 standalone -0.110% vs additive(17%) -0.095% vs 실측 -0.095% → 합연산',
    '서포터 전용 효과 확정': '딜러 캐릭터: 낙인력, 아군피해강화, 아군공격강화 = 전투력 미반영 (서포터 전용)',
  },
  absoluteFormula: {
    '발견 (2026-03-03)': '원문 계수 0.0288은 내부 계산값이며, API CombatPower는 ÷100 스케일링',
    '실제 공식': '전투력(표시값) = 기본공격력 × 0.000288 × Π(1 + 요소%)',
    '동치 표현': '전투력 = (기본공격력 × 0.0288 × Π) ÷ 100',
    '핵심 보정 (2026-03-03)': '진화 1.0714%/P, 깨달음 0.7%/P + 4단계메인×1.231, 도약 0.2%/P (전부 실측 검증)',
    '검증 char_a (보정 후)': [
      '기본공격력 187,064 × 0.000288 = 53.87',
      'Π(에스더/펫 제외) = 91.59 → CP = 4934.29 (오차 -2.63%)',
      'Π(에2-8강+펫상 포함) = 93.04 → CP = 5066.76 (오차 -0.011%)',
      '→ 공식 자체 정확성 확인 (에스더/펫만 API 미제공으로 추정)',
    ],
    '검증 char_b': '139,726 × 0.000288 × Π = 40.24 × Π → 오차 -9.52% (별도 조사 필요)',
    '확인된 모든 요소': [
      '전투레벨(70) +29.45%',
      '무기품질(100) +30.00%',
      '전투스탯(520+225+1841) +77.58%',
      '아크패시브 진화(140P) +70.00%',
      '아크패시브 깨달음(101P) +70.70%',
      '아크패시브 도약(70P) +14.00%',
      '카르마 진화(6랭크) +3.60%',
      '카르마 도약(24Lv) +0.48%',
      '각인 5종 합계 +97.88%',
...
export const RESEARCH_TODO = [
  // ★★★ 검증 완료 (2026-03-03) ★★★
  // ✅ 진화 1.0714%/P (비스탯 노드), 스탯 노드 포인트 기여 0
  // ✅ 깨달음 0.7%/P + 4단계 메인 ×1.231 (ON/OFF)
  // ✅ 도약 0.2%/P (카르마 없음)
  // ✅ 스탯 계수 (치+특+신) × 0.03% 곱연산
  //
  // ✅ 질서_별 코어옵션 전 구간 검증 (고대, 0~20P, 전 구간 0.005% 이내)
  // ✅ 질서_별 고대 = 유물 + 1% (17P↑) 확인
  // ✅ 젬 풀 모델: 추가피해=합연산, 보스피해=독립, 공격력=합연산
  // ✅ 서포터 전용 효과 확정: 낙인력/아군피해강화/아군공격강화 → 딜러 전투력 미반영
  //
  // ★★ 아크그리드 잔여 미완 ★★
  '⓵ 혼돈 코어 테스트: 질서만 확인됨, 혼돈 코어 값 cross-verify 필요 (현재 추정치)',
  '⓶ 예측불가 0P base 재측정: 14P↔15P 불일치 원인 확인',
  // 우선순위 높음
  '아크그리드 질서_해달 테이블 cross-verify: 별은 검증 완료, 해달도 직접 실측 필요',
  // 우선순위 중간
  '장신구 연마 효과 상세 파싱 및 전투력 기여 검증',
  '팔찌 특수 효과의 정확한 전투력 반영 방식',
  'T4 보석 기본공격력% 보너스의 혼합 레벨 계산 방식',
  // 우선순위 낮음
  '에스더 부여 효과 파싱 방법 조사',
  '펫 방범대 효과 파싱 방법 조사',
  '★ 카르마 랭크/레벨 API 제공 확인됨: arkpassive.Points[].Description 파싱 구현 필요',
];
