...
function CharacterPageInner({ initialRanking }: { initialRanking: RankingEntry[] }) {
  ...
  return (
    <Container fluid className={styles.pageContainer}>
      ...
      <GuideFaq
        guideTitle="캐릭터 조회 이용 가이드"
        sections={[
          {
            heading: '전투력이 보여주는 것',
            paragraphs: [
              '전투력은 로스트아크 공식 API가 캐릭터의 실제 세팅을 바탕으로 산출해 내려주는 수치이며, 로아로골은 이 값을 그대로 가져와 보여줄 뿐 별도로 임의 계산을 하지 않습니다. 이 수치에는 장비의 힘·민첩·지능과 무기 공격력 같은 기본 스탯은 물론, 착용한 각인 조합, 보석 효과, 팔찌의 부여효과, 악세서리 연마 옵션, 아크패시브의 진화·깨달음·도약 포인트, 아크그리드에 장착한 코어의 옵션까지 캐릭터를 구성하는 거의 모든 요소가 함께 반영됩니다.',
              '그래서 전투력은 "장비 하나만 바꿔도 세팅 전체의 밸런스가 어떻게 달라졌는지"를 압축해서 보여주는 값에 가깝습니다. 반지 하나를 상급 연마로 바꾸거나 아크그리드 코어 등급을 올리는 것처럼 눈에 잘 띄지 않는 변화도 전투력 수치의 변동으로 확인할 수 있습니다.',
            ],
          },
          {
            heading: '아이템레벨과는 어떻게 다른가요',
            paragraphs: [
              '아이템레벨은 장착한 장비의 등급과 강화 단계만으로 정해지는 값이라, 같은 아이템레벨이어도 각인이나 보석, 아크패시브, 아크그리드 코어 구성이 다르면 실제 전투력은 캐릭터마다 차이가 날 수 있습니다. 반대로 전투력이 비슷한 두 캐릭터라도 아이템레벨은 다를 수 있는데, 한쪽은 장비 강화에, 다른 쪽은 각인·보석·아크그리드 같은 세부 세팅에 더 투자했기 때문입니다. 캐릭터 조회 화면에서는 두 수치를 함께 보여주므로, 장비 강화가 우선인지 세부 세팅 보완이 우선인지 판단하는 데 참고할 수 있습니다.',
            ],
          },
          {
            heading: '캐릭터 랭킹은 어떤 기준으로 매겨지나요',
            paragraphs: [
              '랭킹은 서버 전체 유저를 실시간으로 수집한 목록이 아니라, 로아로골 검색창에서 한 번이라도 조회되어 데이터베이스에 저장된 캐릭터들끼리 비교하는 목록입니다. 내 캐릭터를 랭킹에 올리려면 먼저 검색창에 이름을 입력해 조회하면 됩니다.',
              '기본 정렬은 전투력 내림차순이며, 화면 상단의 전투력·아이템레벨 정렬 버튼을 누르면 내림차순 → 오름차순 → 기본값 순으로 전환됩니다. 여기에 직업·세부 스펙, 딜러·서포터 역할, 보유 칭호(혹한의 군주·홍염의 군주·심연의 군주·돌로리스·이클립스 등), 아크그리드 고대 코어 보유 개수, 아이템레벨 범위까지 조합해서 필터링할 수 있어, 같은 직업군이나 비슷한 스펙끼리 순위를 비교하기에도 활용할 수 있습니다.',
            ],
          },
          {
            heading: '검색 결과 화면에서 함께 확인할 수 있는 정보',
            paragraphs: [
              '캐릭터를 조회하면 전투력·아이템레벨 외에도 장착 장비, 각인 조합, 보석, 팔찌 부여효과, 악세서리 연마 등급, 보주와 초월 단계, 아크패시브 포인트, 아크그리드에 장착한 질서·혼돈 코어, 보유 칭호와 카드 세트, 같은 원정대 소속 캐릭터 목록까지 한 화면에서 확인할 수 있습니다. 세팅을 부위별로 하나씩 눌러보지 않아도 캐릭터의 전반적인 완성도를 한눈에 파악할 수 있도록 구성했습니다.',
            ],
          },
        ]}
        faqs={faqData}
