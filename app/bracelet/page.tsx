          ...
          sections={[
            {
              heading: '팔찌 부여효과란',
              paragraphs: [
                '찬란한 구원자의 팔찌는 치명·특화·신속 중 두 가지를 골라 61~120 사이의 수치를 갖는 고정 특성과, 그와 별개로 굴려서 얻는 부여효과 3개로 구성됩니다. 이 시뮬레이터는 33종의 부여효과를 그룹 단위로 관리하며, 치명타 콤보나 무기 공격력 계열처럼 같은 그룹에 속한 효과는 한 팔찌 안에서 중복으로 붙지 않도록 처리합니다.',
                '처음 팔찌를 생성하면 서로 다른 그룹에서 무작위로 3개 효과가 뽑히고, 이후 재변환을 통해 원하는 조합을 만들어가는 방식으로 진행됩니다.',
              ],
            },
            {
              heading: '효과 등급(일반·희귀·특수)과 확률 구조',
              paragraphs: [
                '부여효과는 common(일반)·rare(희귀)·special(특수) 세 등급으로 나뉘고, 등급마다 뽑힐 확률이 크게 다릅니다. 공격 및 이동 속도 증가 같은 일반 계열은 낮음 4.2%·중간 2.1%·높음 0.7%로 상대적으로 자주 등장하고, 치명타 적중률+치명타 피해 콤보 같은 희귀 계열은 0.5%·0.25%·0.08333%로 훨씬 드물게 나옵니다. 적에게 주는 피해, 치명타 적중률, 치명타 피해처럼 실전에서 많이 찾는 특수 계열은 1.0909%·0.5455%·0.1818%로 그 중간에 위치합니다.',
                '같은 효과 그룹 안에서도 낮음/중간/높음 세 단계로 수치가 나뉘며, 수치가 높은 상위 단계일수록 그 효과가 나올 확률은 더 낮아집니다. 카드에서 색이 강조된 숫자가 바로 이 단계별로 달라지는 부분입니다.',
              ],
            },
            {
              heading: '재변환 기회와 잠금 시스템',
              paragraphs: [
                '팔찌를 생성하면 우선 무료 재변환 기회가 3번 주어지고, 이 기회를 모두 사용한 뒤에는 재변환권을 소모해 최대 3회까지 다시 굴릴 수 있습니다. 현재 몇 번의 기회와 재변환권이 남았는지는 상태 표시줄에서 바로 확인할 수 있습니다.',
                '마음에 드는 효과가 나오면 잠금 버튼으로 고정할 수 있으며, 잠근 효과와 그 효과가 속한 그룹은 이후 재변환에서 제외되어 나머지 자리만 다시 뽑힙니다. 재변환을 실행하면 기존 효과와 새로 뽑힌 신규 효과가 나란히 표시되는 비교 화면으로 넘어가고, 두 카드 중 하나를 선택해 확정하면 남은 기회 범위 안에서 이 과정을 반복할 수 있습니다.',
              ],
            },
            {
              heading: '조합 확률은 어떻게 계산되나요',
              paragraphs: [
                '카드 하단에 표시되는 조합 확률은 현재 붙어 있는 3개 효과가 정확히 그 조합으로 함께 나올 실제 확률입니다. 단순히 개별 확률을 곱한 값이 아니라, 세 효과가 어떤 순서로 뽑히든 상관없이 하나가 정해질 때마다 남은 확률 풀(100%)에서 이미 정해진 효과가 속한 그룹의 전체 확률을 제외하고 재정규화하는 방식으로 계산합니다. 그래서 일반 등급 위주의 무난한 조합은 수 % 대로 나오지만, 희귀 등급 효과가 섞인 조합은 0.01% 이하로 뚝 떨어지기도 합니다.',
              ],
            },
          ]}
          faqs={faqData}
