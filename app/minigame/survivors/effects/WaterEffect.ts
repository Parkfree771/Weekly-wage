...
export class WaterEffect {
  ...
  private draw() {
    ...
    for (const ring of this.rings) {
      ...
      // ── 글로우 (부드러운 배경 빛) ──
      this.glowGfx.lineStyle(baseThickness * 4, 0x1e40af, lifeAlpha * 0.1);
      for (let j = 0; j <= segments; j++) {
        const angle = j * step;
        const undulation =
          Math.sin(angle * waveFreq1 + timePhase + seed) * waveAmp1 +
          Math.sin(angle * waveFreq2 - timePhase * 1.3 + seed * 0.7) * waveAmp2;
        const r = currentR + undulation;
        const x = Math.cos(angle) * r;
        const y = Math.sin(angle) * r;
        if (j === 0) this.glowGfx.moveTo(x, y);
        else this.glowGfx.lineTo(x, y);
      }
