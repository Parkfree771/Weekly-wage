        ...
        <Row className="justify-content-center">
          <Col xl={12} lg={12} md={12}>
            ...
            <GuideFaq
              ...
              sections={[
                {
                  heading: '3단계 구성과 기본 보상·더보기 구조',
                  paragraphs: [
                    '지평의 성당은 아이템 레벨 1700(1단계), 1720(2단계), 1750(3단계) 총 세 단계로 구성되며, 각 단계는 1관문과 2관문 두 개의 관문으로 나뉩니다. 관문을 클리어하면 고정된 클리어 골드와 함께 운명의 파괴석·수호석(고레벨은 결정), 위대한 운명의 돌파석, 운명의 파편, 은총의 파편, 코어를 기본 보상으로 받습니다.',
                    '기본 보상 외에 추가 골드를 지불하는 더보기를 선택하면 같은 종류의 재료를 더 많이 받을 수 있습니다. 로골로골은 더보기로 얻는 재료의 실시간 거래소 가치 합계에서 더보기 소모 골드를 뺀 손익을 관문별로 계산해 보여주므로, 재료 시세가 낮아지는 시기에는 더보기가 오히려 손해로 표시되는 경우도 있습니다.',
                  ],
                },
                {
                  heading: '은총의 파편 상점 — 1회 한정 교환과 무제한 교환',
                  paragraphs: [
                    '관문 클리어로 모은 은총의 파편은 화면 하단 은총의 파편 상점에서 다양한 재화로 교환할 수 있습니다. 유물 코어 선택상자, 고대 코어 선택상자, 초기 영웅 젬 선택상자처럼 캐릭터당 평생 한 번만 교환 가능한 "1회" 항목과, 지평의 야금술·재봉술 선택상자, 지평의 재련 재료 상자, 젬 랜덤 상자, 고대 코어 랜덤 상자처럼 은총의 파편만 있으면 계속 교환할 수 있는 "무제한" 항목으로 나뉩니다.',
                    '각 교환 항목은 요구 레벨이 다르게 설정되어 있어, 예를 들어 유물 코어 선택상자는 Lv.1700부터, 고대 코어 선택상자는 더 높은 레벨부터 열립니다. 상점 목록에서 항목을 선택하면 교환에 필요한 은총의 파편·골드·코어 정수를 실시간 시세로 환산한 총비용과, 코어·젬·재료 상자의 구성 요소별 가치, 교환 효율(받는 가치 대비 지불 비용 비율)까지 함께 확인할 수 있습니다.',
                  ],
                },
                {
                  heading: '은총의 파편 1개는 왜 고정 시세가 아닌가',
                  paragraphs: [
                    '은총의 파편은 거래소·경매장에서 직접 거래되지 않는 재화라 시세 조회가 불가능합니다. 이 때문에 로골로골은 은총의 파편 60개로 교환할 수 있는 지평의 재련 재료 상자(운명의 파괴석 결정, 수호석 결정, 위대한 돌파석, 운명의 파편으로 구성)의 실시간 거래소 가치 총합을 60으로 나누어 은총의 파편 1개의 대체 가치를 역산합니다.',
                    '이렇게 산출한 개당 가치는 관문 클리어·더보기 보상에 포함된 은총의 파편은 물론, 상점에서 은총의 파편으로 교환하는 모든 항목의 골드 환산 비용에도 동일하게 적용되어, 어떤 화면에서 확인하더라도 일관된 기준으로 손익을 비교할 수 있습니다.',
                  ],
                },
                {
                  heading: '주간 교환 달력으로 성당 진행 계획 세우기',
                  paragraphs: [
                    '주간 교환 달력은 2026년 3월 18일을 1주차 시작으로 최대 24주간의 은총의 파편 누적량을 미리 계산해 보여줍니다. 레벨(1700·1720·1750)에 따라 주당 기본 수급량과 더보기 선택 시 수급량이 다르게 설정되어 있어, 캐릭터의 실제 성당 진행 레벨과 매주 더보기 여부에 맞춰 확인할 수 있습니다.',
                    '주차별 카드에서 그 주에 새로 교환 가능해지는 항목을 확인하고 체크하면, 다음 항목까지 필요한 누적 은총의 파편 수와 이미 소비한 양이 자동으로 반영되어 다시 계산됩니다. 이 체크 상태는 로그인 계정이 아니라 사용 중인 브라우저에 저장되므로, 같은 브라우저로 다시 방문하면 계획이 그대로 유지됩니다.',
                  ],
                },
              ]}
              faqs={faqData}
