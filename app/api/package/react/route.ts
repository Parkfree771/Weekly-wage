...
// - 쓰기 최대 1회/요청: 쿠키의 이전 표와 요청한 표를 비교해 달라진 만큼만 upsert 한다.
//   같은 표를 다시 보내면(새로고침·중복 클릭) 아무것도 쓰지 않는다.
// - 응답 stats 에 최신 카운트를 실어 화면이 서버 값으로 되맞춘다(별도 조회 없음).
...
export async function POST(request: NextRequest) {
  try {
    ...
    const stats = await bumpPackageStats(postId, delta);
    ...
    const res = NextResponse.json({ ok: true, prev, next, changed: true, stats });
